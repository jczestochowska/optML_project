...
experiment_state = {"target_accuracy":target_accuracy,"num_rounds": 0, "test_accuracies": [], "conserved_bits_from_server": [], "conserved_bits_from_clients": []}


while testing_accuracy < target_accuracy:

    # set non improvement trigger for "3" communication rounds
    nonImpTrigger = 5
    if experiment_state['test_accuracies']:
        if np.count_nonzero(np.around(experiment_state['test_accuracies']) == np.round(max(experiment_state['test_accuracies']))) >= nonImpTrigger:
            print('The same max accuracy was achieved {0} times without further improvement'.format(nonImpTrigger))
            break
